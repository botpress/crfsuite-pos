...
void NodeTrainer::message(const std::string &msg)
{
  ...
  if (callback != NULL)
  {
    Local<Value> argv[] = {New<String>(msg.c_str()).ToLocalChecked()};
...
void TrainerClass::Init(Local<Object> exports)
{
  ...
  NODE_SET_PROTOTYPE_METHOD(tpl, "set_callback", SetCallback);
...
void TrainerClass::SetCallback(const FunctionCallbackInfo<Value> &args)
{
  Isolate *isolate = args.GetIsolate();
  TrainerClass *obj = ObjectWrap::Unwrap<TrainerClass>(args.Holder());
  Local<Function> cb = Local<Function>::Cast(args[0]);
  obj->trainer->callback = new Nan::Callback(Nan::To<v8::Function>(args[0]).ToLocalChecked());
}
